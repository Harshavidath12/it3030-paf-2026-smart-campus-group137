...
const AdminDashboard = () => {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (userData.role !== 'ADMIN') {
          ...
          setUser(userData);
          fetchUsersList(); // Fetch all users data
        }
      } catch (err) {
  ...
  const handleAddSubmit = async (e) => {
    ...
    try {
      ...
      setNewUser({ name: '', email: '', password: '', role: 'TECHNICIAN' }); // Reset
      fetchUsersList(); // Refresh list immediately
    } catch (err) {
                ...
                {usersList.length === 0 && !loadingUsers ? (
                  ...
                  usersList.map((u, i) => (
                    ...
                    >
                      ...
                      <td style={{ padding: '14px 16px', fontWeight: '500' }}>{u.name || <span style={{color: 'var(--text-muted)', fontStyle: 'italic'}}>Not provided</span>}</td>
                      ...
                      <td style={{ padding: '14px 16px' }}>
                        <span style={{
                          ...
                          background: u.role === 'ADMIN' ? 'rgba(239, 68, 68, 0.15)' : 
                                      u.role === 'MANAGER' ? 'rgba(139, 92, 246, 0.15)' :
                                      u.role === 'TECHNICIAN' ? 'rgba(245, 158, 11, 0.15)' : 
                                      'rgba(59, 130, 246, 0.15)',
                          color: u.role === 'ADMIN' ? '#ef4444' : 
                                 u.role === 'MANAGER' ? '#8b5cf6' :
                                 u.role === 'TECHNICIAN' ? '#f59e0b' : 
                                 'var(--primary-color)'
                        }}>
                          {u.role}
                        </span>
      ...
      {showAddModal && (
        <div style={{
          ...
          <div style={{
            ...
            {addError && <div style={{ color: 'var(--input-error)', background: 'rgba(239, 68, 68, 0.1)', padding: '10px', borderRadius: '8px', marginBottom: '15px', fontSize: '0.9rem' }}>{addError}</div>}
            ...
            <form onSubmit={handleAddSubmit} className="auth-form" style={{ marginTop: '10px' }}>
              ...
              <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '10px', marginTop: '10px' }}>
                <button 
                  ...
                  style={{ background: 'transparent', border: '1px solid var(--border-color)', color: 'var(--text-main)', padding: '10px 16px', borderRadius: '8px', cursor: 'pointer', fontWeight: '500' }}
                >
                  Cancel
                </button>
                <button 
                  type="submit" 
                  disabled={adding}
                  style={{ background: 'var(--primary-gradient)', border: 'none', color: '#fff', padding: '10px 20px', borderRadius: '8px', cursor: 'pointer', fontWeight: '600' }}
                >
